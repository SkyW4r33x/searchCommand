...
is_installed() {
    if [ -d "/usr/local/bin/searchCommand" ] || [ -f "/usr/bin/searchcommand" ] || [ -f "/usr/bin/gtfsearch" ] || [ -d "$USER_HOME/referencestuff" ] || [ -d "$USER_HOME/.data" ] || [ -f "/usr/local/bin/searchCommand.py" ] || [ -f "/usr/local/bin/gtfsearch.py" ]; then
        return 0
    else
        return 1
    fi
...
uninstall_searchcommand() {
    ...
    rm -rf "$USER_HOME/.data" "$USER_HOME/referencestuff" /usr/share/gtfobins /usr/local/bin/searchCommand /usr/bin/searchcommand /usr/bin/gtfsearch /usr/local/bin/searchCommand.py /usr/local/bin/gtfsearch.py &>/dev/null
...
show_installation_summary() {
    ...
    sleep 0.5
    echo -e "[${GREEN}✔${RESET}] Virtual environment: ${BLUE}${BOLD}/usr/local/bin/searchCommand/venv${RESET}"
    sleep 0.5
    echo -e "[${GREEN}✔${RESET}] Main directory: ${BLUE}${BOLD}/usr/local/bin/searchCommand/${RESET}"
    sleep 0.5
...
install_dependencies() {
    ...
    apt install -y python3 python3-venv python3-dev &>/dev/null &
    spinner $! "Installing Python dependencies..." || { log_error "Failed to install dependencies"; return 1; }
}

setup_python_environment() {
    log_info "Setting up Python virtual environment..."
    mkdir -p /usr/local/bin/searchCommand
    python3 -m venv /usr/local/bin/searchCommand/venv
    
    chown -R "$REAL_USER:$REAL_USER" /usr/local/bin/searchCommand/venv
    
    source /usr/local/bin/searchCommand/venv/bin/activate
    
    pip install --upgrade pip &>/dev/null &
    spinner $! "Upgrading pip..." || { log_error "Failed to upgrade pip"; return 1; }
    
    pip install rich prompt-toolkit fuzzywuzzy requests python-Levenshtein &>/dev/null &
    spinner $! "Installing required libraries..." || { log_error "Failed to install libraries"; return 1; }
    
    deactivate
    
    chown -R "$REAL_USER:$REAL_USER" /usr/local/bin/searchCommand/venv
    chmod -R 755 /usr/local/bin/searchCommand/venv
}
...
install_searchcommand() {
    ...
    mkdir -p /usr/local/bin/searchCommand
    cp -r searchCommand/* /usr/local/bin/searchCommand/
    
    chown -R "$REAL_USER:$REAL_USER" /usr/local/bin/searchCommand
    chmod -R 755 /usr/local/bin/searchCommand
...
#!/bin/sh
exec /usr/local/bin/searchCommand/venv/bin/python3 /usr/local/bin/searchCommand/main.py "\$@"
EOF
    chmod +x /usr/bin/searchcommand
    
    cat << EOF > /usr/bin/gtfsearch
#!/bin/sh
exec /usr/local/bin/searchCommand/venv/bin/python3 /usr/local/bin/searchCommand/gtfsearch.py "\$@"
EOF
